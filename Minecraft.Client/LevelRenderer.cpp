...
bool LevelRenderer::updateDirtyChunks()
{
	...
	const size_t maxAtomicNearRebuilds = 2;
#endif
...
//			app.DebugPrintf("[%d,%d,%d]\n",nearestClipChunks.empty(),considered,wouldBeNearButEmpty);
			...
			if( veryNearCount > 0 )
			{
				while(nearestClipChunks.size() > maxAtomicNearRebuilds)
				{
					nearestClipChunks.pop_back();
				}
				if( veryNearCount > (int)maxAtomicNearRebuilds )
				{
					veryNearCount = (int)maxAtomicNearRebuilds;
				}
			}
#endif
		}
