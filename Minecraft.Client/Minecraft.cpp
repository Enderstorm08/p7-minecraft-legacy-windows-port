...
static shared_ptr<MultiplayerLocalPlayer> GetWindowsChatLocalPlayer(Minecraft *minecraft, int iPad)
{
	if (minecraft == NULL || iPad < 0 || iPad >= XUSER_MAX_COUNT)
	{
		return nullptr;
	}
	if (minecraft->localplayers[iPad] != NULL)
	{
		return minecraft->localplayers[iPad];
	}
	return minecraft->player;
}

static shared_ptr<ServerPlayer> GetWindowsCommandSender(Minecraft *minecraft, int iPad)
{
	shared_ptr<MultiplayerLocalPlayer> localPlayer = GetWindowsChatLocalPlayer(minecraft, iPad);
	MinecraftServer *server = MinecraftServer::getInstance();
	if (localPlayer == NULL || server == NULL || server->getPlayers() == NULL)
	{
		return nullptr;
	}

	shared_ptr<ServerPlayer> sender = server->getPlayers()->getPlayer(localPlayer->getXuid());
	if (sender == NULL)
	{
		sender = server->getPlayers()->getPlayer(localPlayer->getName());
	}
	return sender;
}

static void SendWindowsChatFeedback(Minecraft *minecraft, int iPad, shared_ptr<ServerPlayer> sender, const wstring& message)
{
	if (sender != NULL)
	{
		sender->sendMessage(message);
	}
	else if (minecraft != NULL && minecraft->gui != NULL)
	{
		minecraft->gui->addMessage(message, iPad);
	}
}

static void SendWindowsCheatsRequiredFeedback(Minecraft *minecraft, int iPad, shared_ptr<ServerPlayer> sender)
{
    SendWindowsChatFeedback(minecraft, iPad, sender, L"Allow Cheats is off for this world. Turn it on in More Options or Host Options.");
}

static bool TryParseWindowsInt(const wstring& token, int &value)
{
...
static bool KillWindowsCommandSender(Minecraft *minecraft, int iPad, shared_ptr<ServerPlayer> sender)
{
    if (sender == NULL)
    {
        SendWindowsChatFeedback(minecraft, iPad, sender, L"Commands are only available while a world is loaded.");
        return true;
    }

    if (!sender->hasPermission(eGameCommand_Kill))
    {
        SendWindowsCheatsRequiredFeedback(minecraft, iPad, sender);
        return true;
    }

    shared_ptr<Player> player = dynamic_pointer_cast<Player>(sender);
    if (player == NULL || player->removed || player->getHealth() <= 0)
    {
        SendWindowsChatFeedback(minecraft, iPad, sender, L"No living player was available to kill.");
        return true;
    }
...
static int KillWindowsEnderDragonsInLevel(Level *level)
{
    if (level == NULL)
    {
        return 0;
    ...
    {
        shared_ptr<EnderDragon> dragon = dynamic_pointer_cast<EnderDragon>(entities[i]);
        if (dragon != NULL && !dragon->removed)
        {
            dragon->forceCommandKill();
            ++killCount;
        }
    }
...
static bool KillWindowsEnderDragonCommand(Minecraft *minecraft, int iPad, shared_ptr<ServerPlayer> sender)
{
    if (sender == NULL)
    {
        SendWindowsChatFeedback(minecraft, iPad, sender, L"Commands are only available while a world is loaded.");
    ...
    killCount += KillWindowsEnderDragonsInLevel(senderLevel);

    MinecraftServer *server = MinecraftServer::getInstance();
    ServerLevel *endLevel = server != NULL ? server->getLevel(1) : NULL;
    if (endLevel != NULL && endLevel != senderLevel)
    {
        killCount += KillWindowsEnderDragonsInLevel(endLevel);
    }

    if (killCount <= 0)
    {
        SendWindowsChatFeedback(minecraft, iPad, sender, L"No Ender Dragon was found.");
        return true;
    }

    SendWindowsChatFeedback(minecraft, iPad, sender, killCount == 1 ? L"Killed the Ender Dragon." : L"Killed Ender Dragons.");
    return true;
}

static bool DispatchWindowsCommandPacket(Minecraft *minecraft, int iPad, shared_ptr<ServerPlayer> sender, shared_ptr<GameCommandPacket> packet)
{
	MinecraftServer *server = MinecraftServer::getInstance();
	if (sender == NULL || packet == NULL || server == NULL || server->getCommandDispatcher() == NULL)
	{
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Commands are only available in a loaded world.");
		return true;
	}
	server->getCommandDispatcher()->performCommand(sender, packet->command, packet->data);
	return true;
}

static std::vector<std::wstring> TokenizeWindowsCommand(const std::wstring& commandBody)
{
	std::vector<std::wstring> rawTokens = stringSplit(commandBody, L' ');
	std::vector<std::wstring> tokens;
	for (unsigned int i = 0; i < rawTokens.size(); ++i)
	{
		wstring token = trimString(rawTokens[i]);
		if (!token.empty())
		{
...
static bool ExecuteWindowsSlashCommand(Minecraft *minecraft, int iPad, const wstring& chatMessage)
{
	wstring trimmedMessage = trimString(chatMessage);
	if (trimmedMessage.empty() || trimmedMessage[0] != L'/')
	{
		return false;
	}

	std::vector<std::wstring> tokens = TokenizeWindowsCommand(trimmedMessage.substr(1));
	shared_ptr<ServerPlayer> sender = GetWindowsCommandSender(minecraft, iPad);

	if (tokens.empty())
	{
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Try /help for available commands.");
		return true;
	}

	wstring command = toLower(tokens[0]);
	if (command == L"help" || command == L"?")
	{
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Commands: /help, /time day|night, /weather clear|rain, /gamemode survival|creative, /give <id> [count] [data], /kill, /ekill");
		return true;
	}
	...
	if (command == L"time")
	{
        if (!sender->hasPermission(eGameCommand_Time))
        {
            SendWindowsCheatsRequiredFeedback(minecraft, iPad, sender);
            return true;
        }

		wstring mode = tokens.size() >= 2 ? toLower(tokens[1]) : L"";
		if (mode == L"set" && tokens.size() >= 3)
		{
			mode = toLower(tokens[2]);
		}
		if (mode == L"day")
		{
			return DispatchWindowsCommandPacket(minecraft, iPad, sender, TimeCommand::preparePacket(false));
		}
		if (mode == L"night")
		{
			return DispatchWindowsCommandPacket(minecraft, iPad, sender, TimeCommand::preparePacket(true));
		}
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Usage: /time day or /time night");
		return true;
	}

	if (command == L"weather")
	{
		if (!sender->hasPermission(eGameCommand_ToggleDownfall))
		{
			SendWindowsCheatsRequiredFeedback(minecraft, iPad, sender);
			return true;
		}
		...
		if (level == NULL)
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"No overworld is loaded.");
			return true;
		}
		if (mode == L"clear")
		{
			...
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Set the weather to clear.");
			return true;
		}
		if (mode == L"rain")
		{
			level->getLevelData()->setRaining(true);
			level->getLevelData()->setRainTime(6000);
			level->getLevelData()->setThundering(false);
			level->getLevelData()->setThunderTime(0);
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Set the weather to rain.");
			return true;
		}
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Usage: /weather clear or /weather rain");
		return true;
	}

	if (command == L"gamemode")
	{
		if (!sender->hasPermission(eGameCommand_GameMode))
		{
			SendWindowsCheatsRequiredFeedback(minecraft, iPad, sender);
			return true;
		}
		...
		if (gameType == NULL)
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Usage: /gamemode survival or /gamemode creative");
			return true;
		}

		sender->setGameMode(gameType);
		SendWindowsChatFeedback(minecraft, iPad, sender, L"Set own game mode to " + label + L".");
		return true;
	}

	if (command == L"give")
	{
        if (!sender->hasPermission(eGameCommand_Give))
        {
            SendWindowsCheatsRequiredFeedback(minecraft, iPad, sender);
            return true;
        }
		...
		if (tokens.size() < 2)
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Usage: /give <itemId> [count] [data]");
			return true;
		}
		...
		int itemCount = 1;
		int itemAux = 0;
		if (!TryParseWindowsInt(tokens[1], itemId) || itemId < 0 || (unsigned int)itemId >= Item::items.length || Item::items[itemId] == NULL)
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Unknown item id.");
			return true;
		}
		if (tokens.size() >= 3 && !TryParseWindowsInt(tokens[2], itemCount))
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Invalid item count.");
			return true;
		}
		if (tokens.size() >= 4 && !TryParseWindowsInt(tokens[3], itemAux))
		{
			SendWindowsChatFeedback(minecraft, iPad, sender, L"Invalid item data value.");
			return true;
		}
		if (itemCount < 1)
		{
		...
		return DispatchWindowsCommandPacket(minecraft, iPad, sender, GiveItemCommand::preparePacket(sender, itemId, itemCount, itemAux));
	}

	if (command == L"kill")
	{
		return KillWindowsCommandSender(minecraft, iPad, sender);
	}

	if (command == L"ekill")
	{
		return KillWindowsEnderDragonCommand(minecraft, iPad, sender);
	}

	SendWindowsChatFeedback(minecraft, iPad, sender, L"Unknown command. Try /help.");
	return true;
}

static void OpenWindowsChatScreen(Minecraft *minecraft, bool commandPrompt)
{
    ...
    minecraft->setScreen(new ChatScreen(commandPrompt ? L"/" : L""));
}

static bool IsKeyboardActionPressed(EGameAction action)
{
	KeyBind bind = g_KeyBindings.GetBinding(action);
	return bind.isMouse ? g_KeyboardMouseInput.IsMouseButtonPressed(bind.vkCode) : g_KeyboardMouseInput.IsKeyPressed(bind.vkCode);
}

static bool IsKeyboardActionDown(EGameAction action)
{
	KeyBind bind = g_KeyBindings.GetBinding(action);
	return bind.isMouse ? g_KeyboardMouseInput.IsMouseButtonDown(bind.vkCode) : g_KeyboardMouseInput.IsKeyDown(bind.vkCode);
}

static bool IsGameplayActionPressedKeyboard(unsigned char action)
{
	switch(action)
	{
	case MINECRAFT_ACTION_JUMP: return IsKeyboardActionPressed(eGameAction_Jump);
	case MINECRAFT_ACTION_USE: return IsKeyboardActionPressed(eGameAction_UseItem);
	case MINECRAFT_ACTION_INVENTORY: return IsKeyboardActionPressed(eGameAction_Inventory);
	case MINECRAFT_ACTION_ACTION: return IsKeyboardActionPressed(eGameAction_Attack);
	case MINECRAFT_ACTION_CRAFTING: return IsKeyboardActionPressed(eGameAction_Crafting);
	case MINECRAFT_ACTION_PAUSEMENU: return IsKeyboardActionPressed(eGameAction_PauseMenu);
	case MINECRAFT_ACTION_DROP: return IsKeyboardActionPressed(eGameAction_Drop);
	case MINECRAFT_ACTION_SNEAK_TOGGLE: return IsKeyboardActionPressed(eGameAction_Sneak);
	case MINECRAFT_ACTION_RENDER_THIRD_PERSON: return IsKeyboardActionPressed(eGameAction_ToggleThirdPerson);
	case MINECRAFT_ACTION_GAME_INFO: return IsKeyboardActionPressed(eGameAction_TogglePlayerList);
	default: return false;
	}
}

static bool IsGameplayActionDownKeyboard(unsigned char action)
{
	switch(action)
	{
	case MINECRAFT_ACTION_JUMP: return IsKeyboardActionDown(eGameAction_Jump);
	case MINECRAFT_ACTION_USE: return IsKeyboardActionDown(eGameAction_UseItem);
	case MINECRAFT_ACTION_ACTION: return IsKeyboardActionDown(eGameAction_Attack);
	case MINECRAFT_ACTION_SNEAK_TOGGLE: return IsKeyboardActionDown(eGameAction_Sneak);
	default: return false;
	}
}

static int GetKeyboardHotbarSelection()
{
	for(int i = 0; i < 9; ++i)
	{
		if(IsKeyboardActionPressed((EGameAction)(eGameAction_Hotbar1 + i)))
		{
			return i;
		}
	}
...
static bool HasKeyboardMouseGameplayInput()
{
    Minecraft *minecraft = Minecraft::GetInstance();
    if (ui.GetMenuDisplayed(0) || (minecraft != NULL && minecraft->screen != NULL))
    {
        return false;
    }
    return HasImmediateKeyboardMouseInput();
}
#endif
...
void Minecraft::run()
{
	...
	while (running)
	{
		...
		*/
		...
		Sleep(timer->ticks == 0 ? 1 : 0);	// 4J - was Thread.yield()
...
#ifdef _WINDOWS64
						|| (UseKeyboardMouseInput(i) && IsGameplayActionPressedKeyboard(MINECRAFT_ACTION_DROP))
#endif
...
#ifdef _WINDOWS64
							|| (UseKeyboardMouseInput(i) && IsGameplayActionDownKeyboard(MINECRAFT_ACTION_SNEAK_TOGGLE))
#endif
...
#ifdef _WINDOWS64
							|| (UseKeyboardMouseInput(i) && IsGameplayActionPressedKeyboard(MINECRAFT_ACTION_SNEAK_TOGGLE))
#endif
							)		localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE;
					}
					if(InputManager.ButtonPressed(i, MINECRAFT_ACTION_RENDER_THIRD_PERSON)
#ifdef _WINDOWS64
						|| (UseKeyboardMouseInput(i) && IsGameplayActionPressedKeyboard(MINECRAFT_ACTION_RENDER_THIRD_PERSON))
#endif
...
#ifdef _WINDOWS64
						|| (UseKeyboardMouseInput(i) && IsGameplayActionPressedKeyboard(MINECRAFT_ACTION_GAME_INFO))
#endif
...
#ifdef _WINDOWS64
							|| (UseKeyboardMouseInput(idx) && HasKeyboardMouseGameplayInput())
#endif
							)
						{
	...
	*/
	...
	if (screen == NULL && !ui.GetMenuDisplayed(iPad) )
	{
...
#ifdef _WINDOWS64
			|| (UseKeyboardMouseInput(iPad) && g_KeyboardMouseInput.GetMouseWheelDelta() > 0)
#endif
			) && gameMode->isInputAllowed(MINECRAFT_ACTION_LEFT_SCROLL) )
		{
			wheel = 1;
		}
		else if ((InputManager.GetValue(iPad, MINECRAFT_ACTION_RIGHT_SCROLL,true) > 0
#ifdef _WINDOWS64
			|| (UseKeyboardMouseInput(iPad) && g_KeyboardMouseInput.GetMouseWheelDelta() < 0)
#endif
...
#ifdef _WINDOWS64
				|| (UseKeyboardMouseInput(iPad) && IsGameplayActionDownKeyboard(MINECRAFT_ACTION_ACTION))
#endif
		...
		*/
...
#ifdef _WINDOWS64
				&& !(UseKeyboardMouseInput(iPad) && IsGameplayActionDownKeyboard(MINECRAFT_ACTION_USE))
#endif
				...
				if( player->lastClickState == LocalPlayer::lastClick_oldRepeat )
				{
					...
					else
					{
						...
						if ((InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE)
							...
							|| (UseKeyboardMouseInput(iPad) && IsGameplayActionDownKeyboard(MINECRAFT_ACTION_USE))
#endif
							) && ticks - player->lastClickTick[1] >= timer->ticksPerSecond / 4)
						{
...
#ifdef _WINDOWS64
        if (iPad == 0 && !ui.GetMenuDisplayed(iPad) && screen == NULL)
        {
            if (IsKeyboardActionPressed(eGameAction_Command))
            {
                OpenWindowsChatScreen(this, true);
            }
            else if (IsKeyboardActionPressed(eGameAction_Chat))
            {
                OpenWindowsChatScreen(this, false);
            }
        }
#endif
		...
		{
			int keyboardSelected = GetKeyboardHotbarSelection();
			if(keyboardSelected >= 0)
			{
				player->inventory->selected = keyboardSelected;
				selected = true;
			}
		}
#endif
...
{
#ifdef _WINDOWS64
	return ExecuteWindowsSlashCommand(this, 0, chatMessage);
#else
	return false;
#endif
}
