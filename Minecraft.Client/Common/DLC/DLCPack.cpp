...
void DLCPack::addChildPack(DLCPack *childPack)
{
	...
	if(m_bForceOwned)
	{
		childPack->setForceOwned(true, true);
	}
...
void DLCPack::setForceOwned(bool forceOwned, bool recursive /*= true*/)
{
	m_bForceOwned = forceOwned;
	if(recursive)
	{
		for(AUTO_VAR(it, m_childPacks.begin()); it != m_childPacks.end(); ++it)
		{
			(*it)->setForceOwned(forceOwned, true);
		}
	}
}

bool DLCPack::isForceOwned() const
{
	if(m_bForceOwned)
	{
		return true;
	}
	return m_parentPack != NULL && m_parentPack->isForceOwned();
}
