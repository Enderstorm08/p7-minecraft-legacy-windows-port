...
void IUIScene_CraftingMenu::setGroupIndex(int newIndex)
{
	int maxGroups = (m_iContainerType == RECIPE_TYPE_3x3) ? m_iMaxGroup3x3 : m_iMaxGroup2x2;
	if (newIndex < 0 || newIndex >= maxGroups || newIndex == m_iGroupIndex)
		return;

	// Turn off the old group tab
	showTabHighlight(m_iGroupIndex, false);

	m_iGroupIndex = newIndex;
