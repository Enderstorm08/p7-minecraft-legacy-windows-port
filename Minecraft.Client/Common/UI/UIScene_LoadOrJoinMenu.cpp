...
static bool IsBrokenGeneratorLabel(const wstring &label)
{
	wstring trimmed = trimString(label);
	if (trimmed.empty())
	{
		return true;
	}

	if (trimmed.find(L"Default_") == 0)
	{
		return true;
	}
	...
	return numericOnly || !hasAlpha;
}

static wstring ResolveImportedMashupAlias(unsigned int texturePackId)
{
	switch (texturePackId)
	{
	case 402:
		return L"Halo";
	case 400:
		return L"Mass Effect";
	case 401:
		return L"The Elder Scrolls V: Skyrim";
	default:
		return L"";
	}
}

static bool ContainsInsensitive(const wstring &haystack, const wchar_t *needle)
{
	...
	wstring lowerHaystack = haystack;
	for (size_t i = 0; i < lowerHaystack.length(); ++i)
	{
		lowerHaystack[i] = towlower(lowerHaystack[i]);
	}

	wstring lowerNeedle = needle;
	for (size_t i = 0; i < lowerNeedle.length(); ++i)
	{
		lowerNeedle[i] = towlower(lowerNeedle[i]);
	}

	return lowerHaystack.find(lowerNeedle) != wstring::npos;
}

static wstring ResolveImportedMashupAlias(LevelGenerationOptions *levelGen)
{
	if (levelGen == NULL)
	{
		return L"";
	...
	wstring alias = ResolveImportedMashupAlias(levelGen->getRequiredTexturePackId());
	if (!alias.empty())
	{
		return alias;
	}

	wstring grfPath = levelGen->getGrfPath();
	if (ContainsInsensitive(grfPath, L"\\halo\\") || ContainsInsensitive(grfPath, L"/halo/"))
	{
		return L"Halo";
	}
	if (ContainsInsensitive(grfPath, L"mass effect"))
	{
		return L"Mass Effect";
	}
	if (ContainsInsensitive(grfPath, L"skyrim"))
	{
		return L"The Elder Scrolls V: Skyrim";
	}

	wstring defaultSaveName = levelGen->getDefaultSaveName();
	if (ContainsInsensitive(defaultSaveName, L"halo"))
	{
		return L"Halo";
	}
	if (ContainsInsensitive(defaultSaveName, L"mass effect"))
	{
		return L"Mass Effect";
	}
	if (ContainsInsensitive(defaultSaveName, L"skyrim"))
	{
		return L"The Elder Scrolls V: Skyrim";
	}
...
static wstring ResolveStartGameGeneratorLabel(LevelGenerationOptions *levelGen)
{
	if (levelGen == NULL)
	{
		return L"";
	...
	wstring label = levelGen->getDisplayName();
	if (IsBrokenGeneratorLabel(label))
	{
		label = levelGen->getWorldName();
	}

	if (IsBrokenGeneratorLabel(label) && levelGen->getRequiredTexturePackId() != 0)
	{
	...
	if (IsBrokenGeneratorLabel(label))
	{
		label = ResolveImportedMashupAlias(levelGen);
	}

	return IsBrokenGeneratorLabel(label) ? L"" : label;
}
...
void UIScene_LoadOrJoinMenu::AddDefaultButtons()
{
    ...
    for(AUTO_VAR(it, app.getLevelGenerators()->begin()); it != app.getLevelGenerators()->end(); ++it)
    {
		...
		wstring generatorLabel = ResolveStartGameGeneratorLabel(levelGen);
		if (generatorLabel.empty())
		{
			continue;
		}

		m_generators.push_back(levelGen);
		m_buttonListSaves.addItem(generatorLabel);
