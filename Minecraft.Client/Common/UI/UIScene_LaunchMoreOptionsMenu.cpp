...
#ifdef _WINDOWS64
	m_checkboxes[eLaunchCheckbox_HostPrivileges].init(L"Allow Cheats",eLaunchCheckbox_HostPrivileges,m_params->bHostPrivileges);
#else
...
#ifdef _WIN64
	auto isPointInsideRect = [](S32 px, S32 py, S32 x, S32 y, S32 w, S32 h) -> bool
	{
		return w > 0 && h > 0 && px >= x && px <= (x + w) && py >= y && py <= (y + h);
	};

	// Handle mouse click on tab labels to switch tabs
	if (m_loadedResolution == eSceneResolution_1080 && g_KeyboardMouseInput.IsEnabled() && 
		g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
	{
		S32 movieW, movieH;
		...
		S32 mx = (S32)((float)g_KeyboardMouseInput.GetMouseX() * scaleX);
		S32 my = (S32)((float)g_KeyboardMouseInput.GetMouseY() * scaleY);

		// Check Game Options tab label
		S32 gx, gy;
		m_labelGameOptions.getAbsolutePos(gx, gy);
		S32 gw = m_labelGameOptions.getWidth();
		S32 gh = m_labelGameOptions.getHeight();
		S32 gamePadX = gx - 40;
		S32 gamePadY = gy - 18;
		S32 gamePadW = max(gw + 80, movieW / 4);
		S32 gamePadH = max(gh + 36, 56);
		bool gameTabHit = isPointInsideRect(mx, my, gamePadX, gamePadY, gamePadW, gamePadH);
		if (!gameTabHit)
		{
			gameTabHit = isPointInsideRect(mx, my, 0, 0, movieW / 2, 96);
		}
		if (gameTabHit && m_tabIndex != 0)
		{
			m_tabIndex = 0;
			updateTooltips();
			IggyDataValue result;
		...
		// Check World Options tab label
		S32 wx, wy;
		m_labelWorldOptions.getAbsolutePos(wx, wy);
		S32 ww = m_labelWorldOptions.getWidth();
		S32 wh = m_labelWorldOptions.getHeight();
		S32 worldPadX = wx - 40;
		S32 worldPadY = wy - 18;
		S32 worldPadW = max(ww + 80, movieW / 4);
		S32 worldPadH = max(wh + 36, 56);
		bool worldTabHit = isPointInsideRect(mx, my, worldPadX, worldPadY, worldPadW, worldPadH);
		if (!worldTabHit)
		{
			worldTabHit = isPointInsideRect(mx, my, movieW / 2, 0, movieW / 2, 96);
		}
		if (worldTabHit && m_tabIndex != 1)
		{
			m_tabIndex = 1;
			updateTooltips();
			IggyDataValue result;
