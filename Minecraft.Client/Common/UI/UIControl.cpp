...
#ifdef _WIN64
	F64 fwidth = 0.0;
	IggyResult res = IggyValueGetF64RS(getIggyValuePath(), m_nameWidth, NULL, &fwidth);
	if (res == IGGY_RESULT_SUCCESS && fwidth > 0.0)
		m_width = (S32)Math::round(fwidth);
#endif
	return m_width;
}
...
#ifdef _WIN64
	F64 fheight = 0.0;
	IggyResult res = IggyValueGetF64RS(getIggyValuePath(), m_nameHeight, NULL, &fheight);
	if (res == IGGY_RESULT_SUCCESS && fheight > 0.0)
		m_height = (S32)Math::round(fheight);
#endif
	return m_height;
}
...
#ifdef _WIN64
void UIControl::getAbsolutePos(S32 &outX, S32 &outY)
{
	// Query current position from Flash (not cached values)
	F64 fx = 0.0, fy = 0.0;
	IggyValueGetF64RS(getIggyValuePath(), m_nameXPos, NULL, &fx);
	IggyValueGetF64RS(getIggyValuePath(), m_nameYPos, NULL, &fy);
	outX = (S32)fx;
	...
	IggyValuePath* pPath = getIggyValuePath();
	IggyValuePath* pParent = pPath->parent;
	IggyName nameX = m_parentScene->registerFastName(L"x");
	IggyName nameY = m_parentScene->registerFastName(L"y");
	
	for (int level = 0; level < 5 && pParent != NULL; level++)
	{
		F64 parentX = 0.0, parentY = 0.0;
		
		IggyResult resX = IggyValueGetF64RS(pParent, nameX, NULL, &parentX);
		IggyResult resY = IggyValueGetF64RS(pParent, nameY, NULL, &parentY);
		
		if (resX == IGGY_RESULT_SUCCESS)
		{
			outX += (S32)parentX;
		}
		if (resY == IGGY_RESULT_SUCCESS)
		{
			outY += (S32)parentY;
		}
