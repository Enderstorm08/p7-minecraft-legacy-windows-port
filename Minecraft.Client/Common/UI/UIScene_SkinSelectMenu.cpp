...
#ifdef _WIN64
	// Handle mouse click on tab areas to switch skin packs
	if (g_KeyboardMouseInput.IsEnabled() && g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
	{
		S32 movieW, movieH;
		...
		// Check left tab
		S32 lx, ly;
		m_tabLeft.getAbsolutePos(lx, ly);
		S32 lw = m_tabLeft.getWidth();
		S32 lh = m_tabLeft.getHeight();
		if (lw > 0 && lh > 0 && mx >= lx && mx <= lx + lw && my >= ly && my <= ly + lh)
		{
		...
		// Check right tab
		S32 rx, ry;
		m_tabRight.getAbsolutePos(rx, ry);
		S32 rw = m_tabRight.getWidth();
		S32 rh = m_tabRight.getHeight();
		if (rw > 0 && rh > 0 && mx >= rx && mx <= rx + rw && my >= ry && my <= ry + rh)
		{
			ui.PlayUISFX(eSFX_Scroll);
			DWORD startingIndex = m_packIndex;
