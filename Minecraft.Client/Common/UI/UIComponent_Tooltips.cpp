...
bool UIComponent_Tooltips::_SetTooltip(unsigned int iToolTip, int iTextID)
{
	...
	if(iTextID != m_tooltipValues[iToolTip].iString || (iTextID > -1 && !m_tooltipValues[iToolTip].show))
	{
		...
		if(iTextID > -1)
		{
			wstring label = app.GetString(iTextID);
#ifdef _WIN64
			if (m_bKeyboardMode)
			{
				// Prepend the correct keybind label based on tooltip button and context
				EGameAction action = eGameAction_Count;
				switch (iToolTip)
				{
				case eToolTipButtonA:  action = eGameAction_MenuOK; break;
				case eToolTipButtonB:  action = eGameAction_MenuCancel; break;
				case eToolTipButtonX:  action = eGameAction_MenuAlt; break;
				case eToolTipButtonY:
					// Context-dependent: inventory vs quick move
					if (iTextID == IDS_TOOLTIPS_CRAFTING || iTextID == IDS_TOOLTIPS_SHOW_INVENTORY)
						action = eGameAction_Inventory;
					else
						action = eGameAction_MenuQuickMove;
					break;
				case eToolTipButtonLT: action = eGameAction_Attack; break;
				case eToolTipButtonRT: action = eGameAction_UseItem; break;
				case eToolTipButtonLB: action = eGameAction_MenuTabLeft; break;
				case eToolTipButtonRB: action = eGameAction_MenuTabRight; break;
				default: break;
				}
				if (action != eGameAction_Count)
				{
					wstring keyLabel = g_KeyBindings.GetTooltipLabel(action);
					label = keyLabel + L" " + label;
				}
				else if (iToolTip == eToolTipButtonLS)
				{
					label = L"[WASD] " + label;
				}
			}
#endif
			_SetTooltip(iToolTip, label, true);
		}
...
void UIComponent_Tooltips::handleReload()
{
	...
	{
		_SetTooltip(i, m_tooltipValues[i].iString);
		if(!m_tooltipValues[i].show)
		{
			_SetTooltip(i, L"", false, true);
		}
	}
	_Relayout();
}
...
#ifdef _WIN64
void UIComponent_Tooltips::SetKeyboardMode(bool bKeyboardMode)
{
	if (m_bKeyboardMode != bKeyboardMode)
	{
		m_bKeyboardMode = bKeyboardMode;
		// Always tell Flash keyboard mode is OFF so it doesn't prepend
		// hardcoded KEYBOARD_LABELS. C++ handles all keybind label formatting.
		IggyDataValue result;
		IggyDataValue value[1];
		value[0].type = IGGY_DATATYPE_boolean;
		value[0].boolval = false;
		IggyPlayerCallMethodRS(getMovie(), &result, IggyPlayerRootPath(getMovie()), m_funcSetKeyboardMode, 1, value);
	}
}
#endif
