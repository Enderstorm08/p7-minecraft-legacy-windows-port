...
void UIScene_CreativeMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_LEFT_SCROLL:
		if (pressed && specs[m_curTab]->getPageCount() > 1)
		{
	...
	case ACTION_MENU_RIGHT_SCROLL:
		if (pressed && specs[m_curTab]->getPageCount() > 1)
		{
			if (m_tabPage[m_curTab] < (int)specs[m_curTab]->getPageCount() - 1)
			{
				m_tabPage[m_curTab]++;
				switchTab(m_curTab);
			}
...
void UIScene_CreativeMenu::tick()
{
	UIScene_AbstractContainerMenu::tick();

#ifdef _WIN64
	// Handle mouse scroll wheel for page navigation
	if (g_KeyboardMouseInput.IsEnabled())
	{
		int wheelDelta = g_KeyboardMouseInput.GetMouseWheelDelta();
		if (wheelDelta != 0 && specs[m_curTab]->getPageCount() > 1)
		{
			if (wheelDelta < 0 && m_tabPage[m_curTab] < (int)specs[m_curTab]->getPageCount() - 1)
			{
				m_tabPage[m_curTab]++;
				switchTab(m_curTab);
			}
			else if (wheelDelta > 0 && m_tabPage[m_curTab] > 0)
			{
				m_tabPage[m_curTab]--;
				switchTab(m_curTab);
			}
