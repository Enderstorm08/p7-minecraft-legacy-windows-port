...
void UIGroup::WriteSceneStackDebug(FILE *file)
{
	if(file == NULL)
	{
		return;
	...
	fprintf(file, "  Group %d: menu=%d pause=%d container=%d ignoreAutosave=%d ignoreJoin=%d\n",
		(int)m_group,
		m_bMenuDisplayed ? 1 : 0,
		m_bPauseMenuDisplayed ? 1 : 0,
		m_bContainerMenuDisplayed ? 1 : 0,
		m_bIgnoreAutosaveMenuDisplayed ? 1 : 0,
		m_bIgnorePlayerJoinMenuDisplayed ? 1 : 0);

	for(unsigned int i = 0; i < eUILayer_COUNT; ++i)
	{
		m_layers[i]->WriteSceneStackDebug(file);
	}
}
