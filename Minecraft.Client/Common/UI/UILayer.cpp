...
bool UILayer::NavigateToScene(int iPad, EUIScene scene, void *initData)
{
	...
	case eUIScene_SettingsControlMenu:
		...
		break;
#ifdef _WIN64
	case eUIScene_SettingsKeyBindMenu:
		newScene = new UIScene_SettingsKeyBindMenu(iPad, initData, this);
		break;
#endif
...
{
	for(int sceneIndex = (int)m_sceneStack.size() - 1; sceneIndex >= 0; --sceneIndex)
	{
		UIScene *scene = m_sceneStack[sceneIndex];
		if(scene->hasFocus(iPad) && scene->canHandleInput())
		{
			...
			// Navigating back can mutate the scene stack; treat the focused scene as the terminal input target.
			bool sceneStillInStack = std::find(m_sceneStack.begin(), m_sceneStack.end(), scene) != m_sceneStack.end();
			bool consumesLowerScenes = !sceneStillInStack || scene->hidesLowerScenes();
			handled = handled || consumesLowerScenes || (pressed && key == ACTION_MENU_CANCEL);
			break;
		}
...
void UILayer::WriteSceneStackDebug(FILE *file)
{
	if(file == NULL)
	{
		return;
	...
	fprintf(file, "    Layer %d: focus=%d menu=%d pause=%d container=%d count=%u",
#ifdef __PSVITA__
		(int)m_iLayer,
#else
		-1,
#endif
		m_hasFocus ? 1 : 0,
		m_bMenuDisplayed ? 1 : 0,
		m_bPauseMenuDisplayed ? 1 : 0,
		m_bContainerMenuDisplayed ? 1 : 0,
		(unsigned int)m_sceneStack.size());
	...
	for(size_t i = 0; i < m_sceneStack.size(); ++i)
	{
		...
		fprintf(file, "%d", (int)scene->getSceneType());
		if(scene->hasFocus(0))
		{
			fprintf(file, "*");
		}
		fprintf(file, "{ctrl=%d child=%d}", scene->getControlFocus(), scene->getControlChildFocus());
	}
	fprintf(file, "]\n");
}
