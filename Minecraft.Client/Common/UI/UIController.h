...
private:
	...
	int m_inputDispatchDepth;

	static const int UI_BREADCRUMB_COUNT = 32;
	...
	vector<PendingNavigationRequest> m_pendingNavigation;
	vector<PendingMouseAction> m_pendingMouseActions;
	string m_uiBreadcrumbs[UI_BREADCRUMB_COUNT];
	int m_uiBreadcrumbHead;
	int m_uiBreadcrumbCount;
...
#ifdef _WIN64
	// Mouse hover navigation for Windows
	int m_lastMouseX;
	int m_lastMouseY;
	bool m_bMouseOverClickable;  // True when mouse is over a clickable control
	UIScene* m_pMouseHoverScene;  // Scene containing the currently hovered control
	int m_iMouseHoverControlId;   // ID of the currently hovered control (-1 if none)
	UIScene* m_pMouseFocusScene;  // Scene containing the current committed focus target
	int m_iMouseFocusControlId;   // ID of the current committed focus target (-1 if none)
	UIScene* m_pMouseActiveScene; // Scene containing the pressed control
	int m_iMouseActiveControlId;  // ID of the pressed control (-1 if none)
	int m_iMousePressStartX;
	int m_iMousePressStartY;
	bool m_bMouseDragActive;
	void ResetMouseHoverState(bool resetLastMousePos = false);
	void HandleMouseHover();
	bool IsMouseOverClickableControl();
	void ResetMouseActiveState();
	void ResetMouseFocusState();
	UIScene *FindTopMouseScene();
	UIControl *FindSceneControlById(UIScene *pScene, int controlId);
	bool ResolveMouseControlBounds(UIScene *pScene, UIControl *pControl, S32 &x1, S32 &y1, S32 &x2, S32 &y2);
	bool HandleMouseControlRelease(UIScene *pScene, UIControl *pControl, UIControl *pHoveredControl, S32 uiMouseX, S32 uiMouseY);
	void HandleMouseControlDrag(UIScene *pScene, UIControl *pControl, S32 uiMouseX, S32 uiMouseY, bool leftDown);
	void HandleMouseControlHover(UIScene *pScene, UIControl *pControl, S32 uiMouseX, S32 uiMouseY, bool leftDown, bool commitFocus);
	void CommitMouseFocus(UIScene *pScene, UIControl *pControl);
	void QueueMouseActivate(UIScene *pScene, UIControl *pControl, F64 childId);
	void QueueMouseCancel(UIScene *pScene);
	void QueueMouseDispatchInput(UIScene *pScene, unsigned int key);
	void FlushPendingMouseActions();
	void DispatchMouseSyntheticInput(unsigned int iPad, unsigned int key, bool pressed = true, bool released = true);
#endif
...
public:
	...
	void PushDebugBreadcrumb(const char *szFormat, ...);
	void WriteWindowsDebugSnapshot(FILE *file);
...
private:
	void PushDebugBreadcrumbV(const char *szFormat, va_list args);
	void DispatchInputToGroups(unsigned int iPad, unsigned int key, bool repeat, bool pressed, bool released);
	void FlushPendingNavigation();
	bool NavigateToSceneImmediate(int iPad, EUIScene scene, void *initData, EUILayer layer, EUIGroup group);
	bool NavigateBackImmediate(int iPad, bool forceUsePad, EUIScene eScene, EUILayer eLayer);
	void CloseUIScenesImmediate(int iPad, bool forceIPad);
