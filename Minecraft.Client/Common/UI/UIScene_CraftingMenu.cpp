...
#ifdef _WIN64
	m_pointerPos.x = 960.0f;
	m_pointerPos.y = 540.0f;
	m_fPointerMinX = 0.0f;
	m_fPointerMaxX = 1920.0f;
	m_fPointerMinY = 0.0f;
	m_fPointerMaxY = 1080.0f;
#endif
...
#ifdef _WIN64
	if (m_iPad == 0 && g_KeyboardMouseInput.IsEnabled())
	{
		...
		IggyMakeEventMouseMove(&mouseEvent, (S32)m_pointerPos.x, (S32)m_pointerPos.y);
		IggyEventResult result;
		IggyPlayerDispatchEventRS(getMovie(), &mouseEvent, &result);

		IggyDataValue cursorResult;
		IggyDataValue cursorArgs[2];
		cursorArgs[0].type = IGGY_DATATYPE_number;
		cursorArgs[0].number = (double)m_pointerPos.x;
		cursorArgs[1].type = IGGY_DATATYPE_number;
		cursorArgs[1].number = (double)m_pointerPos.y;
		IggyPlayerCallMethodRS(getMovie(), &cursorResult, IggyPlayerRootPath(getMovie()), m_funcSetCursorPosition, 2, cursorArgs);

		handleMouseHover();

		if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
			handleMouseClick();
...
#ifdef _WIN64
void UIScene_CraftingMenu::updateMousePointer()
{
	S32 movieW, movieH;
	getMovieDimensions(movieW, movieH);
	...
	m_pointerPos.x = (float)g_KeyboardMouseInput.GetMouseX() * scaleX;
	m_pointerPos.y = (float)g_KeyboardMouseInput.GetMouseY() * scaleY;

	if (m_pointerPos.x < m_fPointerMinX) m_pointerPos.x = m_fPointerMinX;
	else if (m_pointerPos.x > m_fPointerMaxX) m_pointerPos.x = m_fPointerMaxX;
	if (m_pointerPos.y < m_fPointerMinY) m_pointerPos.y = m_fPointerMinY;
	else if (m_pointerPos.y > m_fPointerMaxY) m_pointerPos.y = m_fPointerMaxY;
}

void UIScene_CraftingMenu::handleMouseHover()
{
	S32 hSlotX = m_slotListCraftingHSlots.getXPos();
	S32 hSlotY = m_slotListCraftingHSlots.getYPos();
	S32 hSlotW = m_slotListCraftingHSlots.getWidth();
	S32 hSlotH = m_slotListCraftingHSlots.getHeight();

	if (m_pointerPos.x >= hSlotX && m_pointerPos.x < hSlotX + hSlotW &&
		m_pointerPos.y >= hSlotY && m_pointerPos.y < hSlotY + hSlotH)
	{
		int slotWidth = hSlotW / m_iCraftablesMaxHSlotC;
		int i = (int)((m_pointerPos.x - hSlotX) / slotWidth);
		if (i >= 0 && i < m_iCraftablesMaxHSlotC)
		{
			if (m_iCurrentSlotHIndex != i && CanBeMadeA[i].iCount > 0)
			{
				int iOldHSlot = m_iCurrentSlotHIndex;
				m_iCurrentSlotHIndex = i;
	...
	bool bNoScrollSlots = m_bSplitscreen || (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen());

	if (!bNoScrollSlots && CanBeMadeA[m_iCurrentSlotHIndex].iCount > 2)
	{
		for (int i = 0; i < 3; i++)
		{
			S32 vSlotX = m_slotListCrafting3VSlots[i].getXPos();
			S32 vSlotY = m_slotListCrafting3VSlots[i].getYPos();
			S32 vSlotW = m_slotListCrafting3VSlots[i].getWidth();
			S32 vSlotH = m_slotListCrafting3VSlots[i].getHeight();

			if (m_pointerPos.x >= vSlotX && m_pointerPos.x < vSlotX + vSlotW &&
				m_pointerPos.y >= vSlotY && m_pointerPos.y < vSlotY + vSlotH)
			{
				if (m_iCurrentSlotVIndex != i)
				{
					m_iCurrentSlotVIndex = i;
					UpdateHighlight();
					ui.PlayUISFX(eSFX_Focus);
				}
				return;
...
void UIScene_CraftingMenu::handleMouseClick()
{
	S32 mainPanelX = m_controlMainPanel.getXPos();
	S32 mainPanelY = m_controlMainPanel.getYPos();
	S32 mainPanelW = m_controlMainPanel.getWidth();

	S32 tabAreaHeight = 60;
	if (m_pointerPos.x >= mainPanelX && m_pointerPos.x < mainPanelX + mainPanelW &&
		m_pointerPos.y >= mainPanelY && m_pointerPos.y < mainPanelY + tabAreaHeight)
	{
		int maxGroups = getMaxGroups();
		int tabWidth = mainPanelW / maxGroups;
		int clickedTab = (int)((m_pointerPos.x - mainPanelX) / tabWidth);
		if (clickedTab >= 0 && clickedTab < maxGroups)
			setGroupIndex(clickedTab);
		return;
	}
...
void UIScene_CraftingMenu::handleMouseWheel(int delta)
{
	if (CanBeMadeA[m_iCurrentSlotHIndex].iCount <= 1)
		return;

	int totalVariants = CanBeMadeA[m_iCurrentSlotHIndex].iCount;
	int currentSlot = iVSlotIndexA[m_iCurrentSlotVIndex];

	if (delta > 0 && currentSlot > 0)
		currentSlot--;
	else if (delta < 0 && currentSlot < totalVariants - 1)
		currentSlot++;
	else
		return;

	iVSlotIndexA[m_iCurrentSlotVIndex] = currentSlot;
	if (m_iCurrentSlotVIndex == 1)
	{
		iVSlotIndexA[0] = (currentSlot > 0) ? currentSlot - 1 : totalVariants - 1;
		iVSlotIndexA[2] = (currentSlot < totalVariants - 1) ? currentSlot + 1 : 0;
	}
	UpdateVerticalSlots();
	UpdateHighlight();
	ui.PlayUISFX(eSFX_Focus);
...
void UIScene_CraftingMenu::render(S32 width, S32 height, C4JRender::eViewportType viewport)
{
...
void UIScene_CraftingMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	if (iPad == 0 && g_KeyboardMouseInput.IsEnabled())
	{
		if (g_KeyboardMouseInput.IsMouseCaptured())
		{
			if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
			{
				handled = handleKeyDown(m_iPad, ACTION_MENU_A, false);
			}
			if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_RIGHT))
			{
		...
		if (wheelDelta > 0)
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_LEFT_SCROLL, false);
		}
		else if (wheelDelta < 0)
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_RIGHT_SCROLL, false);
		}
		
		// WASD keyboard navigation for crafting slots
		if (g_KeyboardMouseInput.IsKeyPressed(KB_KEY_W))
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_UP, false);
		}
		if (g_KeyboardMouseInput.IsKeyPressed(KB_KEY_S))
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_DOWN, false);
		}
		if (g_KeyboardMouseInput.IsKeyPressed(KB_KEY_A))
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_LEFT, false);
		}
		if (g_KeyboardMouseInput.IsKeyPressed(KB_KEY_D))
		{
			handled = handleKeyDown(m_iPad, ACTION_MENU_RIGHT, false);
		}
	}
#endif
