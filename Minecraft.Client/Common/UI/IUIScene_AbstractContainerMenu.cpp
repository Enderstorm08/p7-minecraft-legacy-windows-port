...
#ifdef _WIN64
	m_fMouseSmoothX = 0.0f;
	m_fMouseSmoothY = 0.0f;
#endif
}
...
#ifdef _WIN64
	if (iPad == 0 && g_KeyboardMouseInput.IsEnabled() && g_KeyboardMouseInput.IsMouseCaptured())
	{
		float mouseDeltaX = g_KeyboardMouseInput.GetMouseDeltaX();
		float mouseDeltaY = g_KeyboardMouseInput.GetMouseDeltaY();

		const float kSmooth = 0.55f;
		const float kSpeed = 0.65f;
		const float kDecay = 0.7f;

		m_fMouseSmoothX = m_fMouseSmoothX * kDecay + mouseDeltaX * kSmooth;
		m_fMouseSmoothY = m_fMouseSmoothY * kDecay + mouseDeltaY * kSmooth;

		if (fabs(m_fMouseSmoothX) > 0.01f || fabs(m_fMouseSmoothY) > 0.01f)
		{
			vPointerPos.x += m_fMouseSmoothX * kSpeed;
			vPointerPos.y += m_fMouseSmoothY * kSpeed;
		...
		if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
			handleKeyDown(iPad, ACTION_MENU_A, false);
		if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_RIGHT))
			handleKeyDown(iPad, ACTION_MENU_X, false);
	}
#endif
