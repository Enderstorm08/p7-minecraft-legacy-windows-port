...
void UIScene::tick()
{
	...
	if(m_opacityLerp != m_targetOpacity)
	{
		m_opacityLerp += (m_targetOpacity - m_opacityLerp) * 0.35f;
		if(fabsf(m_targetOpacity - m_opacityLerp) < 0.01f)
		{
			m_opacityLerp = m_targetOpacity;
		}
		setOpacity(m_opacityLerp);
	}
...
{
#ifdef _WIN64
	// Windows interactive scenes no longer depend on root-movie SetAlpha because a number of
	// Legacy movies do not implement it, which made scene transitions crash-prone on back/cancel.
	if(stealsFocus())
	{
...
void UIScene::navigateBack()
{
	...
	ui.PushDebugBreadcrumb("scene navigateBack scene=%d pad=%d focusCtrl=%d", getSceneType(), m_iPad, getControlFocus());

	bool navigated = ui.NavigateBack(m_iPad);
	if(!navigated)
	{
		app.DebugPrintf("UI navigateBack failed for scene %d on pad %d\n", getSceneType(), m_iPad);
	}
...
void UIScene::gainFocus()
{
	if( !bHasFocus && stealsFocus() )
	{
		...
		*/
		...
		m_targetOpacity = 1.0f;
		setOpacity(m_opacityLerp);
		ui.PushDebugBreadcrumb("scene gainFocus scene=%d pad=%d", getSceneType(), m_iPad);
...
void UIScene::loseFocus()
{
	if(bHasFocus)
	{
		...
		*/
		...
		m_targetOpacity = 1.0f;
		m_opacityLerp = 1.0f;
		ui.PushDebugBreadcrumb("scene loseFocus scene=%d pad=%d", getSceneType(), m_iPad);
		handleLoseFocus();
	}
}
...
void UIScene::TriggerControlPress(F64 controlId, F64 childId)
{
	handlePress(controlId, childId);
}

void UIScene::TriggerDirectMenuAction(int key)
{
	bool handled = false;
	handleInput(m_iPad, key, false, true, false, handled);
}
