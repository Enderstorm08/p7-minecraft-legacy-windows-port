...
#ifdef _WINDOWS64
	m_checkboxHostPrivileges.init(L"Allow Cheats", eControl_HostPrivileges, app.GetGameHostOption(eGameHostOption_CheatsEnabled)!=0);
#else
	m_checkboxHostPrivileges.init(app.GetString(IDS_HOST_PRIVILEGES), eControl_HostPrivileges, app.GetGameHostOption(eGameHostOption_CheatsEnabled)!=0);
#endif
...
void UIScene_InGameHostOptionsMenu::handleInput(int iPad, int key, bool repeat, bool pressed, bool released, bool &handled)
{
	...
	case ACTION_MENU_CANCEL:
		if(pressed)
		{
			...
			app.SetGameHostOption(hostOptions,eGameHostOption_CheatsEnabled,m_checkboxHostPrivileges.IsChecked());
