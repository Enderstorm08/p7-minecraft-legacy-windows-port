...
#endif
{
	...
	SetGameSettings(iPad,eGameSetting_SprintMode,0);
...
void CMinecraftApp::ApplyGameSettingsChanged(int iPad)
{
	...
	ActionGameSettings(iPad,eGameSetting_SprintMode);
...
void CMinecraftApp::ActionGameSettings(int iPad,eGameSetting eVal)
{
	...
	case eGameSetting_ViewBob:
		if(iPad==ProfileManager.GetPrimaryPad())
		{
			pMinecraft->options->bobView = (GameSettingsA[iPad]->usBitmaskValues&0x0004)!=0;
		}
		break;
...
// Interim TU 1.6.6
	case eGameSetting_Sensitivity_InMenu:
		...
		if(iPad == ProfileManager.GetPrimaryPad())
		{
			g_KeyboardMouseInput.SetMouseSensitivity(((float)GameSettingsA[iPad]->ucMenuSensitivity)/100.0f);
		}
#endif
	...
	case eGameSetting_PSVita_NetworkModeAdhoc:
		//nothing to do here
		break;
	case eGameSetting_SprintMode:
		// Windows keyboard sprint mode is read directly from game settings.
		break;
	}
}
...
void CMinecraftApp::SetGameSettings(int iPad,eGameSetting eVal,unsigned char ucVal)
{
	...
	case eGameSetting_PSVita_NetworkModeAdhoc:
		...
		break;	
	case eGameSetting_SprintMode:
		if((GameSettingsA[iPad]->uiBitmaskValues&GAMESETTING_SPRINTMODETOGGLE)!=(ucVal&0x01)<<18)
		{
			if(ucVal==1)
			{
				GameSettingsA[iPad]->uiBitmaskValues|=GAMESETTING_SPRINTMODETOGGLE;
			}
			else
			{
				GameSettingsA[iPad]->uiBitmaskValues&=~GAMESETTING_SPRINTMODETOGGLE;
...
unsigned char CMinecraftApp::GetGameSettings(int iPad,eGameSetting eVal)
{
	...
	case eGameSetting_SprintMode:
		return (GameSettingsA[iPad]->uiBitmaskValues&GAMESETTING_SPRINTMODETOGGLE)>>18;
