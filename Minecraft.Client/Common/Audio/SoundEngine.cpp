...
#ifdef _WINDOWS64
static bool Win64SoundPathExistsA(const std::string &path, bool directory)
{
	...
	return directory ? ((attrs & FILE_ATTRIBUTE_DIRECTORY) != 0) : ((attrs & FILE_ATTRIBUTE_DIRECTORY) == 0);
}

static std::string Win64JoinPathA(const std::string &left, const std::string &right)
{
	if (left.empty())
	{
		return right;
...
static std::string Win64EnsureTrailingSlashA(const std::string &path)
{
	if (path.empty())
	{
		return path;
...
static std::string Win64GetExeDirectoryA()
{
...
static bool Win64ResolveFirstExistingFile(std::string &resolvedPath, const std::string *candidates, size_t candidateCount)
{
	for (size_t i = 0; i < candidateCount; ++i)
	{
		if (Win64SoundPathExistsA(candidates[i], false))
		{
...
static bool Win64ResolveFirstExistingDirectory(std::string &resolvedPath, const std::string *candidates, size_t candidateCount)
{
	for (size_t i = 0; i < candidateCount; ++i)
	{
		if (Win64SoundPathExistsA(candidates[i], true))
		{
			resolvedPath = Win64EnsureTrailingSlashA(candidates[i]);
			return true;
		}
	}
	return false;
...
static std::string Win64NormalizePathA(const std::string &path)
{
	char buffer[MAX_PATH];
...
static void Win64AppendSoundBankCandidate(std::vector<std::string> &candidates, const std::string &path)
{
	const std::string normalizedPath = Win64NormalizePathA(path);
	if (!Win64SoundPathExistsA(normalizedPath, false))
	{
		return;
	}
	...
	{
		if (_stricmp(candidates[i].c_str(), normalizedPath.c_str()) == 0)
		{
			return;
		}
	}
...
static void Win64GetSoundBankCandidates(std::vector<std::string> &candidates)
{
	const std::string exeDir = Win64GetExeDirectoryA();
	const std::string clientRoot = Win64NormalizePathA(Win64JoinPathA(exeDir, "..\\.."));

	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(exeDir, "Sound\\Minecraft.msscmp"));
	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(exeDir, "Minecraft.msscmp"));
	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(clientRoot, "Durango\\Sound\\Minecraft.msscmp"));
	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(clientRoot, "DurangoMedia\\Sound\\Minecraft.msscmp"));
	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(clientRoot, "PS3\\Sound\\Minecraft.msscmp"));
	Win64AppendSoundBankCandidate(candidates, Win64JoinPathA(clientRoot, "PSVita\\Sound\\Minecraft.msscmp"));
}

static bool Win64ResolveMusicBasePath(std::string &resolvedPath)
{
	const std::string exeDir = Win64GetExeDirectoryA();
	const std::string candidates[] =
	{
		Win64JoinPathA(exeDir, "music")
	};
	return Win64ResolveFirstExistingDirectory(resolvedPath, candidates, sizeof(candidates) / sizeof(candidates[0]));
}

static bool Win64ResolveRedistPath(std::string &resolvedPath)
{
	const std::string exeDir = Win64GetExeDirectoryA();
	const std::string candidates[] =
	{
		Win64JoinPathA(exeDir, "redist64")
	};
	return Win64ResolveFirstExistingDirectory(resolvedPath, candidates, sizeof(candidates) / sizeof(candidates[0]));
}
#endif
...
void SoundEngine::init(Options *pOptions)
{
	...
	if (Win64ResolveRedistPath(redistDirectory))
	{
		app.DebugPrintf("SoundEngine::init - resolved redist directory: %s\n", redistDirectory.c_str());
		redistpath = AIL_set_redist_directory((char *)redistDirectory.c_str());
	}
	else
	{
		app.DebugPrintf("SoundEngine::init - using fallback redist directory: %s\n", m_szRedistName);
		redistpath = AIL_set_redist_directory(m_szRedistName);
	}
	app.DebugPrintf("SoundEngine::init - AIL_set_redist_directory returned: %s\n", redistpath ? redistpath : "(null)");
#else
	redistpath=AIL_set_redist_directory(m_szRedistName);
#endif
#endif
	...
	S32 ret = AIL_startup();
	app.DebugPrintf("SoundEngine::init - AIL_startup result: %d\n", ret);
...
#ifdef _WINDOWS64
	std::vector<std::string> bankCandidates;
	Win64GetSoundBankCandidates(bankCandidates);
	if (bankCandidates.empty())
	{
		std::string fallbackPath = Win64NormalizePathA(Win64JoinPathA(Win64GetExeDirectoryA(), "Sound\\Minecraft.msscmp"));
		strcpy_s(szBankName, fallbackPath.c_str());
		bankCandidates.push_back(fallbackPath);
	}

	for (size_t bankIndex = 0; bankIndex < bankCandidates.size() && m_hBank == NULL; ++bankIndex)
	{
		strcpy_s(szBankName, bankCandidates[bankIndex].c_str());
		app.DebugPrintf("SoundEngine::init - attempting sound bank candidate [%u/%u]: %s\n",
			(unsigned int)(bankIndex + 1), (unsigned int)bankCandidates.size(), szBankName);
		m_hBank = AIL_add_soundbank(szBankName, 0);
		if (m_hBank == NULL)
		{
			app.DebugPrintf("SoundEngine::init - failed sound bank candidate: %s (%s)\n",
	...
	if(m_hBank == NULL)
	{
		...
		app.DebugPrintf("SoundEngine::init - continuing without a sound bank; streaming music may still work\n");
	}
	else
	{
		app.DebugPrintf("SoundEngine::init - selected sound bank: %s\n", szBankName);
		//#ifdef _DEBUG
...
int SoundEngine::OpenStreamThreadProc( void* lpParameter )
{
	...
	{
		app.DebugPrintf("SoundEngine::OpenStreamThreadProc - failed to open stream: %s (%s)\n", soundEngine->m_szStreamName, AIL_last_error());
	}
	else
	{
		app.DebugPrintf("SoundEngine::OpenStreamThreadProc - opened stream: %s\n", soundEngine->m_szStreamName);
	}
	return 0;
}
...
#else
#ifdef _WINDOWS64
			std::string resolvedMusicBase;
			if (Win64ResolveMusicBasePath(resolvedMusicBase))
			{
				strcpy_s(m_szStreamName, resolvedMusicBase.c_str());
			}
			else
			{
				strcpy_s(m_szStreamName, m_szMusicPath);
			}
			app.DebugPrintf("SoundEngine::playMusicUpdate - music base path: %s\n", m_szStreamName);
#else
