...
{
	wstring message = trimString(packet->m_stringArgs.size() > 0 ? packet->m_stringArgs[0] : L"");
	if (message.empty())
	{
		return;
	}
	...
	{
		player->sendMessage(L"Chat message too long.");
		return;
	}
	...
	for (unsigned int i = 0; i < message.length(); ++i)
	{
		...
		if (ch < 32 && ch != L' ')
		{
			player->sendMessage(L"Illegal characters in chat.");
			return;
		}
	}
	...
	{
		player->sendMessage(L"Use the in-game chat bar to enter slash commands.");
		return;
	}

	wstring displayName = player->getDisplayName();
	if (displayName.empty())
	{
		displayName = player->getName();
	}

	server->getPlayers()->broadcastAll(shared_ptr<Packet>(new ChatPacket(L"<" + displayName + L"> " + message)));
...
void PlayerConnection::handleServerSettingsChanged(shared_ptr<ServerSettingsChangedPacket> packet)
{
	if(packet->action==ServerSettingsChangedPacket::HOST_IN_GAME_SETTINGS)
	{
		...
		if( (networkPlayer != NULL && networkPlayer->IsHost()) || player->isModerator())
		{
            ...
            app.SetGameHostOption(eGameHostOption_CheatsEnabled, app.GetGameHostOption(packet->data,eGameHostOption_CheatsEnabled));

            bool allowCommands = app.GetGameHostOption(eGameHostOption_CheatsEnabled) != 0;
            if(server != NULL)
            {
                for(unsigned int i = 0; i < server->levels.length; ++i)
                {
                    ...
                    {
                        server->levels[i]->getLevelData()->setAllowCommands(allowCommands);
                    }
                }
            }
