...
{
#ifdef _WINDOWS64
	int windowWidth = g_KeyboardMouseInput.GetWindowWidth();
	int windowHeight = g_KeyboardMouseInput.GetWindowHeight();
	if (windowWidth <= 0) windowWidth = 1;
	if (windowHeight <= 0) windowHeight = 1;

	int xm = (g_KeyboardMouseInput.GetMouseX() * width) / windowWidth;
	int ym = (g_KeyboardMouseInput.GetMouseY() * height) / windowHeight;

	if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
	{
		mouseClicked(xm, ym, 0);
	}
	if (g_KeyboardMouseInput.IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
	{
		mouseReleased(xm, ym, 0);
	}
	if (g_KeyboardMouseInput.IsMouseButtonPressed(MOUSE_BUTTON_RIGHT))
	{
		mouseClicked(xm, ym, 1);
	}
	if (g_KeyboardMouseInput.IsMouseButtonReleased(MOUSE_BUTTON_RIGHT))
	{
		mouseReleased(xm, ym, 1);
	}

	int vkCode = 0;
	while (g_KeyboardMouseInput.PopScreenKey(vkCode))
	{
		switch (vkCode)
		{
		case VK_ESCAPE:
	...
	wchar_t ch = 0;
	while (g_KeyboardMouseInput.PopCharacter(ch))
	{
		if (ch != L'\r' && ch != L'\b' && ch != L'\t')
		{
			keyPressed(ch, 0);
		}
	}
