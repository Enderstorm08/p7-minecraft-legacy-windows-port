...
static bool IsKeyboardSprintBindingDown()
{
	KeyBind sprintBind = g_KeyBindings.GetBinding(eGameAction_Sprint);
	return sprintBind.isMouse ? g_KeyboardMouseInput.IsMouseButtonDown(sprintBind.vkCode) : g_KeyboardMouseInput.IsKeyDown(sprintBind.vkCode);
}

static bool UseWindowsKeyboardSprintMode(LocalPlayer *player)
{
	if (player == NULL || player->GetXboxPad() != 0 || ui.GetMenuDisplayed(0))
	{
		return false;
	}

	return g_KeyboardMouseInput.GetLastInputDevice() == KeyboardMouseInput::eInputDevice_Keyboard || IsKeyboardSprintBindingDown();
}
#endif
...
void LocalPlayer::serverAiStep()
{
	...
	yBobO = yBob;
	xBobO = xBob;
	xBob += (xRot - xBob) * 0.35f;
	yBob += (yRot - yBob) * 0.35f;
...
void LocalPlayer::aiStep()
{
...
#ifdef _WINDOWS64
	bool useKeyboardSprintMode = UseWindowsKeyboardSprintMode(this);
	bool sprintToggleMode = app.GetGameSettings(m_iPad, eGameSetting_SprintMode) != 0;
	bool sprintHoldActive = useKeyboardSprintMode && !sprintToggleMode;
	bool sprintHeld = sprintHoldActive && IsKeyboardSprintBindingDown();
#endif
	...
	if (onGround && !isSprinting() && enoughFoodToSprint && !isUsingItem() && !hasEffect(MobEffect::blindness))
	{
		...
		if (sprintHoldActive)
		{
			if (sprintHeld && input->ya >= runTreshold)
			{
				setSprinting(true);
				sprintTriggerTime = 0;
				sprintTriggerRegisteredReturn = false;
	...
	if (sprintHoldActive)
	{
		if (isSprinting() && (!sprintHeld || input->ya < runTreshold || !enoughFoodToSprint))
		{
			setSprinting(false);
		}
	}
...
{
	wstring trimmedMessage = trimString(message);
	if (trimmedMessage.empty())
	{
		return;
	}

	wstring displayName = getDisplayName();
	if (displayName.empty())
	{
		displayName = getName();
	}

	wstring chatLine = L"<" + displayName + L"> " + trimmedMessage;
	MinecraftServer *server = MinecraftServer::getInstance();
	if (server != NULL && server->getPlayers() != NULL)
	{
		server->getPlayers()->broadcastAll(shared_ptr<Packet>(new ChatPacket(chatLine)));
	}
	else if (minecraft != NULL && minecraft->gui != NULL)
	{
		minecraft->gui->addMessage(chatLine, GetXboxPad());
	}
}
