...
static C_4JProfile::PROFILESETTINGS ProfileSettingsA[XUSER_MAX_COUNT];
static int( *defaultOptionsCallback)(LPVOID,C_4JProfile::PROFILESETTINGS *, const int iPad) = NULL;
static LPVOID lpProfileParam = NULL;
static int s_windowsProfileDataBytes = 0;
static unsigned short s_windowsProfileVersion = 0;
static unsigned int *s_pWindowsGameDefinedDataChangedBitmask = NULL;

namespace
{
	struct WindowsProfileFileHeader
	{
		DWORD magic;
		unsigned short version;
		unsigned short reserved;
		DWORD profileCount;
		DWORD profileSettingsBytes;
		DWORD gameDefinedBytes;
	};

	static const DWORD WINDOWS_PROFILE_MAGIC = 0x314A344D; // M4J1

	static std::wstring GetWindowsProfilePath()
	{
		...
		path += L"windows_profile_v10.bin";
		return path;
	}

	static void ApplyDefaultWindowsProfileData(byte *pBytes)
	{
		if (pBytes == NULL)
		{
			return;
		...
		GAME_SETTINGS *pGameSettings = (GAME_SETTINGS *)pBytes;
	...
	static bool LoadWindowsProfileFile()
	{
		if (s_windowsProfileDataBytes <= 0)
		{
			return false;
		}

		FILE *fp = _wfopen(GetWindowsProfilePath().c_str(), L"rb");
		if (fp == NULL)
		{
			return false;
		}

		WindowsProfileFileHeader header = { 0 };
		bool success = false;
		if (fread(&header, sizeof(header), 1, fp) == 1 &&
			header.magic == WINDOWS_PROFILE_MAGIC &&
			header.profileCount == XUSER_MAX_COUNT &&
			header.profileSettingsBytes == sizeof(C_4JProfile::PROFILESETTINGS) &&
			header.gameDefinedBytes == (DWORD)s_windowsProfileDataBytes)
		{
			success = true;
			for (int i = 0; i < XUSER_MAX_COUNT; ++i)
			{
				if (fread(&ProfileSettingsA[i], sizeof(C_4JProfile::PROFILESETTINGS), 1, fp) != 1)
				{
					success = false;
					break;
				}
			...
			for (int i = 0; success && i < XUSER_MAX_COUNT; ++i)
			{
				if (fread(profileData[i], s_windowsProfileDataBytes, 1, fp) != 1)
				{
					success = false;
					break;
				}
		...
		if (success && s_pWindowsGameDefinedDataChangedBitmask != NULL)
		{
			*s_pWindowsGameDefinedDataChangedBitmask = (1u << XUSER_MAX_COUNT) - 1u;
		}

		return success;
	}

	static void SaveWindowsProfileFile()
	{
		if (s_windowsProfileDataBytes <= 0)
		{
			return;
		...
		FILE *fp = _wfopen(GetWindowsProfilePath().c_str(), L"wb");
		if (fp == NULL)
		{
			return;
		}
		...
		WindowsProfileFileHeader header = { 0 };
		header.magic = WINDOWS_PROFILE_MAGIC;
		header.version = s_windowsProfileVersion;
		header.profileCount = XUSER_MAX_COUNT;
		header.profileSettingsBytes = sizeof(C_4JProfile::PROFILESETTINGS);
		header.gameDefinedBytes = s_windowsProfileDataBytes;

		fwrite(&header, sizeof(header), 1, fp);
		for (int i = 0; i < XUSER_MAX_COUNT; ++i)
		{
			fwrite(&ProfileSettingsA[i], sizeof(C_4JProfile::PROFILESETTINGS), 1, fp);
		}
		for (int i = 0; i < XUSER_MAX_COUNT; ++i)
		{
			fwrite(profileData[i], s_windowsProfileDataBytes, 1, fp);
		}
...
{
	s_windowsProfileVersion = usProfileVersion;
	s_windowsProfileDataBytes = iGameDefinedDataSizeX4 / XUSER_MAX_COUNT;
	s_pWindowsGameDefinedDataChangedBitmask = puiGameDefinedDataChangedBitmask;

	for( int i = 0; i < 4; i++ )
	{
		profileData[i] = new byte[s_windowsProfileDataBytes];
		ZeroMemory(profileData[i], sizeof(byte) * s_windowsProfileDataBytes);
		ZeroMemory(&ProfileSettingsA[i], sizeof(C_4JProfile::PROFILESETTINGS));
		ApplyDefaultWindowsProfileData((byte *)profileData[i]);
		if (defaultOptionsCallback != NULL)
		{
			defaultOptionsCallback(lpProfileParam, &ProfileSettingsA[i], i);
		}
	}

	LoadWindowsProfileFile();
}
...
void				C_4JProfile::WriteToProfile(int iQuadrant, bool bGameDefinedDataChanged, bool bOverride5MinuteLimitOnProfileWrites) { SaveWindowsProfileFile(); }
void				C_4JProfile::ForceQueuedProfileWrites(int iPad) { SaveWindowsProfileFile(); }
...
#ifdef _WINDOWS64
static std::wstring g_windowsRememberedSaveTitle = L"";

void Windows64RememberSaveTitle(LPCWSTR pwchDefaultSaveName)
{
	g_windowsRememberedSaveTitle = (pwchDefaultSaveName != NULL) ? pwchDefaultSaveName : L"";
}

LPCWSTR Windows64GetRememberedSaveTitle()
{
	return g_windowsRememberedSaveTitle.c_str();
}

LPCWSTR C4JStorage::GetSaveTitle()
{
	return Windows64GetRememberedSaveTitle();
}
#endif
