...
//            try {
				...
				if (cmds[0] == L"smoothCamera") smoothCamera = cmds[1]==L"true";
...
//        PrintWriter pw = new PrintWriter(new FileWriter(optionsFile));
        ...
        dos.writeChars(L"mouseSensitivity:" + _toString<float>(sensitivity) + L"\n");
		dos.writeChars(L"fov:" + _toString<float>(fov) + L"\n");
		dos.writeChars(L"gamma:" + _toString<float>(gamma) + L"\n");
        dos.writeChars(L"viewDistance:" + _toString<int>(viewDistance) + L"\n");
        dos.writeChars(L"guiScale:" + _toString<int>(guiScale) + L"\n");
		dos.writeChars(L"particles:" + _toString<int>(particles) + L"\n");
        dos.writeChars(L"bobView:" + wstring(bobView ? L"true" : L"false") + L"\n");
        dos.writeChars(L"smoothCamera:" + wstring(smoothCamera ? L"true" : L"false") + L"\n");
        ...
        dos.writeChars(L"fpsLimit:" + _toString<int>(framerateLimit) + L"\n");
        dos.writeChars(L"difficulty:" + _toString<int>(difficulty) + L"\n");
        dos.writeChars(L"fancyGraphics:" + wstring(fancyGraphics ? L"true" : L"false") + L"\n");
        dos.writeChars(L"ao:" + wstring(ambientOcclusion ? L"true" : L"false") + L"\n");
		dos.writeChars(L"clouds:" + wstring(renderClouds ? L"true" : L"false") + L"\n");
        dos.writeChars(L"skin:" + skin + L"\n");
        dos.writeChars(L"lastServer:" + lastMpIp + L"\n");

        for (int i = 0; i < keyMappings_length; i++)
		{
            dos.writeChars(L"key_" + keyMappings[i]->name + L":" + _toString<int>(keyMappings[i]->key) + L"\n");
        }
