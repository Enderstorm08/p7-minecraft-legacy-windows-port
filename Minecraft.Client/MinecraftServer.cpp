...
bool MinecraftServer::initServer(__int64 seed, NetworkGameInitData *initData, DWORD initSettings, bool findSeed)
{
    ...
    t.start();
        ...
        wstring levelName = trimString(StorageManager.GetSaveTitle());
        if (levelName.empty())
        {
            levelName = settings->getString(L"level-name", L"world");
        }
...
//    }
	...
	if( app.GetGameHostOption(eGameHostOption_BonusChest ) ) levelSettings->enableStartingBonusItems();
        if( app.GetGameHostOption(eGameHostOption_CheatsEnabled ) ) levelSettings->enableSinglePlayerCommands();
...
//		m_lastSentDifficulty = pMinecraft->options->difficulty;
        ...
        levels[i]->getLevelData()->setAllowCommands(app.GetGameHostOption(eGameHostOption_CheatsEnabled) != 0);
