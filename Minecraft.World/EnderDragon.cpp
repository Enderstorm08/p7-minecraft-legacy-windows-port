...
namespace
{
	const int FIRST_END_GATEWAY_X = 8;
	const int FIRST_END_GATEWAY_Z = 0;
	const int FIRST_OUTER_END_GATEWAY_X = 128;
	const int FIRST_OUTER_END_GATEWAY_Z = 0;

	void BuildEndGateway(Level *level, int x, int y, int z)
	{
		...
		level->setTile(x, y, z, Tile::endGateway_Id);
		TheEndPortal::allowAnywhere(false);
	}

	void SpawnInitialEndGatewayPair(Level *level)
	{
		...
		int sourceY = max(level->seaLevel + 3, level->getTopSolidBlock(FIRST_END_GATEWAY_X, FIRST_END_GATEWAY_Z));
		int targetY = max(level->seaLevel + 6, level->getTopSolidBlock(FIRST_OUTER_END_GATEWAY_X, FIRST_OUTER_END_GATEWAY_Z));

		BuildEndGateway(level, FIRST_END_GATEWAY_X, sourceY, FIRST_END_GATEWAY_Z);
		BuildEndGateway(level, FIRST_OUTER_END_GATEWAY_X, targetY, FIRST_OUTER_END_GATEWAY_Z);

		levelData->setHasEndGateway(true);
		levelData->setXEndGateway(FIRST_END_GATEWAY_X);
		levelData->setYEndGateway(sourceY);
		levelData->setZEndGateway(FIRST_END_GATEWAY_Z);
		levelData->setXOuterEndGateway(FIRST_OUTER_END_GATEWAY_X);
		levelData->setYOuterEndGateway(targetY);
		levelData->setZOuterEndGateway(FIRST_OUTER_END_GATEWAY_Z);
	}
}
...
void EnderDragon::forceCommandKill()
{
	if (health <= 0)
	{
		return;
...
void EnderDragon::tickDeath()
{
	...
	if (dragonDeathTime == 200 && !level->isClientSide)
	{
		...
		SpawnInitialEndGatewayPair(level);
		remove();
	}
}
