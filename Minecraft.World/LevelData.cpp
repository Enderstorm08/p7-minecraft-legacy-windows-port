...
LevelData::LevelData()
{
	bEndGatewayUnlocked = false;
	xEndGateway = yEndGateway = zEndGateway = 0;
	xOuterEndGateway = yOuterEndGateway = zOuterEndGateway = 0;
}
...
LevelData::LevelData(CompoundTag *tag)
{
	...
	bEndGatewayUnlocked = tag->getBoolean(L"hasEndGateway");
	if (!bEndGatewayUnlocked)
	{
		xEndGateway = yEndGateway = zEndGateway = 0;
		xOuterEndGateway = yOuterEndGateway = zOuterEndGateway = 0;
	}
	...
	{
		xEndGateway = tag->getInt(L"EndGatewayX");
		yEndGateway = tag->getInt(L"EndGatewayY");
		zEndGateway = tag->getInt(L"EndGatewayZ");
		xOuterEndGateway = tag->getInt(L"OuterEndGatewayX");
		yOuterEndGateway = tag->getInt(L"OuterEndGatewayY");
		zOuterEndGateway = tag->getInt(L"OuterEndGatewayZ");
	}
...
//    this->loadedPlayerTag = NULL;	// 4J - we don't store this anymore
	...
	this->bEndGatewayUnlocked = false;
	this->xEndGateway = this->yEndGateway = this->zEndGateway = 0;
	this->xOuterEndGateway = this->yOuterEndGateway = this->zOuterEndGateway = 0;
...
//    this->loadedPlayerTag = copy->loadedPlayerTag;		// 4J - we don't store this anymore
	...
	this->bEndGatewayUnlocked = copy->bEndGatewayUnlocked;
	this->xEndGateway = copy->xEndGateway;
	this->yEndGateway = copy->yEndGateway;
	this->zEndGateway = copy->zEndGateway;
	this->xOuterEndGateway = copy->xOuterEndGateway;
	this->yOuterEndGateway = copy->yOuterEndGateway;
	this->zOuterEndGateway = copy->zOuterEndGateway;
...
void LevelData::setTagData(CompoundTag *tag) 
{
	...
	tag->putBoolean(L"hasEndGateway", bEndGatewayUnlocked);
	tag->putInt(L"EndGatewayX", xEndGateway);
	tag->putInt(L"EndGatewayY", yEndGateway);
	tag->putInt(L"EndGatewayZ", zEndGateway);
	tag->putInt(L"OuterEndGatewayX", xOuterEndGateway);
	tag->putInt(L"OuterEndGatewayY", yOuterEndGateway);
	tag->putInt(L"OuterEndGatewayZ", zOuterEndGateway);
...
bool LevelData::getHasEndGateway()
{
	return bEndGatewayUnlocked;
}

int LevelData::getXEndGateway()
{
	return xEndGateway;
}

int LevelData::getYEndGateway()
{
	return yEndGateway;
}

int LevelData::getZEndGateway()
{
	return zEndGateway;
}

int LevelData::getXOuterEndGateway()
{
	return xOuterEndGateway;
}

int LevelData::getYOuterEndGateway()
{
	return yOuterEndGateway;
}

int LevelData::getZOuterEndGateway()
{
	return zOuterEndGateway;
}
...
void LevelData::setHasEndGateway(bool hasEndGateway)
{
	this->bEndGatewayUnlocked = hasEndGateway;
}

void LevelData::setXEndGateway(int xEndGateway)
{
	this->xEndGateway = xEndGateway;
}

void LevelData::setYEndGateway(int yEndGateway)
{
	this->yEndGateway = yEndGateway;
}

void LevelData::setZEndGateway(int zEndGateway)
{
	this->zEndGateway = zEndGateway;
}

void LevelData::setXOuterEndGateway(int xOuterEndGateway)
{
	this->xOuterEndGateway = xOuterEndGateway;
}

void LevelData::setYOuterEndGateway(int yOuterEndGateway)
{
	this->yOuterEndGateway = yOuterEndGateway;
}

void LevelData::setZOuterEndGateway(int zOuterEndGateway)
{
	this->zOuterEndGateway = zOuterEndGateway;
}
